    ...
    for (var i = 0; i < arguments.length; i++) {
        if (i == 0) {
            if (typeof (arguments[i]) === "string") {
                parameters.push({ animate: false });
            }
        }
        parameters.push(arguments[i]);
    }
    cordova.exec(null, null, "NativeControls", "showTabBarItems", parameters);
};
...
NativeControls.prototype.createActionSheet = function(buttonTitles, actionSheetTitle, cancelButtonIndex, destructiveButtonIndex) {
    ...
    cordova.exec(null, null, "NativeControls", "createActionSheet", options, params);
